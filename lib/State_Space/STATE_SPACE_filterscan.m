...
syms siL_d_s siL_q_s svpoc_d_s svpoc_q_s svvsc_d_s svvsc_q_s sig_d_s sig_q_s ...
     somega sL_vsc sR_vsc sC_filt somega_b sR_g sL_g svg_d svg_q 
...
x = [siL_d_s; ...                   % Filter inductor current (d-axis)
     ...
     svpoc_q_s;
     sig_d_s;
     sig_q_s];                    % Filter capacitor voltage (q-axis) 

u = [svvsc_d_s; ...                 % VSC output voltage (d-axis)
     svvsc_q_s; ...                 % VSC output voltage (q-axis)
     svg_d; ...                   % Grid current (d-axis)
     svg_q];                      % Grid current (q-axis)
...
params = [sL_vsc; ...               % VSC filter inductance (pu)
          ...
          somega;
          sL_g;
          sR_g];                   % Operating angular frequency   
...
% Filter inductor voltage equation (d-axis)
f5 = (svpoc_d_s - svg_d - sR_g*sig_d_s + somega*sL_g*sig_q_s)*somega_b/sL_g;

% Filter inductor voltage equation (q-axis)  
f6 = (svpoc_q_s - svg_q - sR_g*sig_q_s - somega*sL_g*sig_d_s)*somega_b/sL_g;
...
h = [siL_d_s; siL_q_s; svpoc_d_s; svpoc_q_s; sig_d_s; sig_q_s];  % Output: all filter states
...
x_eq = [iL_d_s; iL_q_s; vpoc_d_s; vpoc_q_s; ig_d_s; ig_q_s];   % Steady-state filter states

% Equilibrium input variables
u_eq = [vvsc_d_s; vvsc_q_s; 1; 0];   % Steady-state inputs
...
params_val = [L_vsc/L_base; ...    % Normalized filter inductance
              ...
              1;
              L_grid/L_base;
              R_grid/Z_base];                   % Normalized operating frequency
...
% % Symbolic variables: filter currents/voltages (dq frame) and parameters
% syms siL_d_s siL_q_s svpoc_d_s svpoc_q_s svvsc_d_s svvsc_q_s sig_d_s sig_q_s ...
%      somega sL_vsc sR_vsc sC_filt somega_b svg_d svg_q sR_0
% 
% % ============================================================
% % 2. Definition of state, input, and parameter vectors
% % ============================================================
...
% 
% % Filter capacitor current equation (d-axis)
% sig_d_s = (svpoc_d_s - svg_d)/sR_0;
% f3 = (siL_d_s - sig_d_s + somega*sC_filt*svpoc_q_s)*somega_b/sC_filt;
% 
% % Filter capacitor current equation (q-axis)
% sig_q_s = (svpoc_q_s - svg_q)/sR_0;
% f4 = (siL_q_s - sig_q_s - somega*sC_filt*svpoc_d_s)*somega_b/sC_filt;
% 
% f = [f1; f2; f3; f4];
% 
...
% 
% % Equilibrium input variables
% u_eq = [vvsc_d_s; vvsc_q_s; vvsc_d_s; vvsc_q_s];   % Steady-state inputs
% 
% % Parameter values (per-unit normalized)
% params_val = [L_vsc/L_base; ...    % Normalized filter inductance
%               R_vsc/Z_base; ...    % Normalized filter resistance
